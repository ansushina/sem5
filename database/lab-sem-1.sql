...
alter table ODS_1.Company add constraint pk_company primary key (store_code);
alter table ODS_1.StoreRooms add constraint pk_storerooms primary key (store_code);

alter table ODS_1.Delivery add constraint fk_del_comp foreign key (del_company) references ODS_1.Company (store_code);
alter table ODS_1.Delivery add constraint fk_del_store foreign key (del_destination) references ODS_1.StoreRooms (store_code);
...
select * from sys.all_objects 
where name like '%Rooms';

select * from sys.columns 
where object_id in (select object_id from sys.all_objects where schema_id=5);

select * from sys.all_objects 
where type='u';

select * from sys.Objects where name='pk_company';

select tbl.name , pk_company.*
 from sys.Objects pk_company
 inner join 
  sys.Objects tbl
 on pk_company.parent_object_id = tbl.Object_id where pk_company.type in ('PK', 'f');