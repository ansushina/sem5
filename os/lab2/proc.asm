.386P ; (1)Раsрешение коwаид МП 386 и 486 
; Структура для опксAHия дескрипторов сегментов 
...
attr_2 db 0 ; (7)ГрAHица (биты 16 ... 19) и атрибуты 2 
base_h db 0 ; (8)База, биты 24 ... 31 
descr ends  ; (9)

data segment ; (10) Начапо сегмента дAHных 
; Таблица глобальных дескриnторов GDT 
gdt_null descr <0,0,0,0,0,0>; (11);Hyneaoй дескриnтор 
gdt_data descr <data_size-1,0,0,92h>; (12)Ceл-р 8, сеrмент дAHных 
...
s      db ?
data_size=$-gdt_null ; (21) Размер сегмента дAHных 
...
main proc ; (25) 
	...
	mov DS, AX 					; (28) адрес сеrмеита дAHных 
; Вычислим 32-битовый линейный адрес сеrмента даииых и загрузим ero 
; в дескриптор сеrмеита дAHных а GDT. 
...
go: mov EAX,CR0 ; (881По.пучим' содер:кимое CR0 
	...
	mov SS, AX ;  ( 971 адресуемость стека ;Раsрешим аnпаратике и иеwаскируамые преркваии~ 
